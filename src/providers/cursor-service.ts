...
export class CursorService implements IRecentProjectProvider {
    ...
    private async findStoragePath(): Promise<string | null> {
        ...
        if (process.platform === 'darwin') {
            ...
            possiblePaths = [
                ...
                path.join(appSupport, 'Cursor/User/storage.json'),
                path.join(appSupport, 'Cursor/storage.json'),
            ];
        } else if (process.platform === 'linux') {
            possiblePaths = [
                path.join(home, '.config/Cursor/User/globalStorage/storage.json'),
                path.join(home, '.config/Cursor/User/storage.json'),
            ...
            possiblePaths = [
                ...
                path.join(appData, 'Cursor/User/storage.json'),
                path.join(appData, 'Cursor/storage.json'),
            ];
        }
        for (const p of possiblePaths) {
            if (fs.existsSync(p)) {
                logger.debug(`[Cursor] 找到存储文件: ${p}`);
                return p;
            }
        }
        return null;
        ...
        try {
            ...
            if (storagePath.endsWith('.json')) {
                ...
                projects = projects.map(p => {
                    ...
                    return { ...p, path: normalized, source: 'cursor' };
                });
                // 去重
                const unique = Array.from(new Map(projects.map(p => [p.path, p])).values());
                return unique;
            }
            logger.debug(`[Cursor] 检测到不受支持的存储文件格式: ${storagePath}`);
            return [];
        } catch (error) {
            logger.error('[Cursor] 获取最近项目列表失败:', error);
            return [];
        }
    }
    ...
    private async parseJsonStorage(filePath: string): Promise<RecentProject[]> {
        try {
                ...
                if (recentMenu?.submenu?.items) {
                    for (const item of recentMenu.submenu.items) {
                        if (item.id === 'openRecentFolder' && item.uri && item.label) {
                            ...
                            if (projectPath) {
                                projects.push({ name: item.label, path: projectPath, source: 'cursor' });
                            }
                        }
                    }
                }
            ...
            if (projects.length > 0) {
                logger.debug(`[Cursor] 通过解析 MenubarData 成功提取了 ${projects.length} 个项目`);
                return Array.from(new Map(projects.map(p => [p.path, p])).values());
            }
            // Fallback Method: backupWorkspaces
            logger.debug('[Cursor] MenubarData 解析失败或为空，尝试使用 backupWorkspaces 作为备用方案');
            ...
            if (projects.length > 0) {
                logger.debug(`[Cursor] 通过解析 backupWorkspaces 成功提取了 ${projects.length} 个项目`);
                return Array.from(new Map(projects.map(p => [p.path, p])).values());
            }
            logger.info(`[Cursor] 在 ${filePath} 中未能找到任何有效的项目列表结构。`);
            return [];
        } catch (error) {
            logger.error(`[Cursor] 解析Cursor storage.json失败: ${filePath}`, error);
            return [];
        }
    }
    ...
    private uriToPath(uriString: string): string | null {
        ...
        try {
            ...
            logger.error(`[Cursor] URI解析失败: ${uriString}`, e);
            return null;
        }
    }
} 