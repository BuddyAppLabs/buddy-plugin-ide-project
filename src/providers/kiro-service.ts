...
export class KiroService implements IRecentProjectProvider {
    ...
    private async findStoragePath(): Promise<string | null> {
        ...
        if (process.platform === 'darwin') {
            ...
            possiblePaths = [
                path.join(appSupport, 'Kiro/User/globalStorage/storage.json'),
                path.join(appSupport, 'Kiro/User/storage.json'),
                path.join(appSupport, 'Kiro/storage.json'),
            ];
        } else if (process.platform === 'linux') {
            possiblePaths = [
                path.join(home, '.config/Kiro/User/globalStorage/storage.json'),
                path.join(home, '.config/Kiro/User/storage.json'),
            ...
            possiblePaths = [
                ...
                path.join(appData, 'Kiro/User/storage.json'),
                path.join(appData, 'Kiro/storage.json'),
            ];
        }
        for (const p of possiblePaths) {
            if (fs.existsSync(p)) {
                logger.debug(`[Kiro] 找到存储文件: ${p}`);
                return p;
            }
        }
        return null;
        ...
        try {
            ...
            if (storagePath.endsWith('.json')) {
                ...
                projects = projects.map(p => {
                    ...
                    return { ...p, path: normalized, source: 'kiro' };
                });
                // 去重
                const unique = Array.from(new Map(projects.map(p => [p.path, p])).values());
                return unique;
            }
            logger.debug(`[Kiro] 检测到不受支持的存储文件格式: ${storagePath}`);
            return [];
        } catch (error) {
            logger.error('[Kiro] 获取最近项目列表失败:', error);
            return [];
        }
    }
    ...
    private async parseJsonStorage(filePath: string): Promise<RecentProject[]> {
        try {
                ...
                if (recentMenu?.submenu?.items) {
                    for (const item of recentMenu.submenu.items) {
                        if (item.id === 'openRecentFolder' && item.uri && item.label) {
                            ...
                            if (projectPath) {
                                projects.push({ name: item.label, path: projectPath, source: 'kiro' });
                            }
                        }
                    }
                }
            ...
            if (projects.length > 0) {
                logger.debug(`[Kiro] 通过解析 MenubarData 成功提取了 ${projects.length} 个项目`);
                return Array.from(new Map(projects.map(p => [p.path, p])).values());
            }
            // Fallback Method: backupWorkspaces
            logger.debug('[Kiro] MenubarData 解析失败或为空，尝试使用 backupWorkspaces 作为备用方案');
            ...
            if (projects.length > 0) {
                logger.debug(`[Kiro] 通过解析 backupWorkspaces 成功提取了 ${projects.length} 个项目`);
                return Array.from(new Map(projects.map(p => [p.path, p])).values());
            }
            logger.info(`[Kiro] 在 ${filePath} 中未能找到任何有效的项目列表结构。`);
            return [];
        } catch (error) {
            logger.error(`[Kiro] 解析Kiro storage.json失败: ${filePath}`, error);
            return [];
        }
    }
    ...
    private uriToPath(uriString: string): string | null {
        ...
        try {
            ...
            logger.error(`[Kiro] URI解析失败: ${uriString}`, e);
            return null;
        }
    }
} 